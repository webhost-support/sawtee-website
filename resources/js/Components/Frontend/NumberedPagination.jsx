...
import { HiChevronRight, HiChevronLeft, HiArrowLeft, HiArrowRight } from 'react-icons/hi';
...
const NumberedPagination = ({ links, prevPage, nextPage, currentPage, totalPages }) => {
	...
	return (
		<Stack
			...
			mt={{ base: 3, md: 0 }}
		>
			<Box onMouseEnter={() => setHover([!hover[0], hover[1]])} onMouseLeave={() => setHover([!hover[0], hover[1]])}>
				<StyledLink href={prevPage}>
					<PaginationButton isDisabled={!prevPage} leftIcon={!hover[0] ? <HiChevronLeft /> : <HiArrowLeft />}>
						Newer Posts
					</PaginationButton>
				</StyledLink>
			</Box>
			<Stack direction="row">
				{paginationArray.map((item, index) => {
					// if item is dots, "..."
					if (item === '...') {
						return <PaginationButton key={`${item} + 1`}>{`...`}</PaginationButton>;
					}

					return (
						<StyledLink key={`${item} + 1`} href={links[index + 1].url}>
							<PaginationButton isActive={links[index + 1].active}>{item}</PaginationButton>
						</StyledLink>
					);
				})}
			</Stack>
			<Box onMouseEnter={() => setHover([hover[0], !hover[1]])} onMouseLeave={() => setHover([hover[0], !hover[1]])}>
				<StyledLink href={nextPage}>
					<PaginationButton isDisabled={!nextPage} rightIcon={!hover[1] ? <HiChevronRight /> : <HiArrowRight />}>
						Older Posts
					</PaginationButton>
				</StyledLink>
			</Box>
...
const PaginationButton = ({ children, isDisabled, isActive, ...rest }) => {
	...
	const ButtonColor = useColorModeValue('var(--color-dark)', 'var(--color-light)');
