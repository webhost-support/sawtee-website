import { Stack, Box, useColorModeValue, Icon, Flex, Text, useDisclosure, Collapse } from '@chakra-ui/react';
					...
					{menuItem.children &&
						menuItem.children.map((child, index) => (
							<React.Fragment key={child.title}>
								<DropDownMenu menuItem={child} index={index} padding={'5px'} size="sm" />
							</React.Fragment>
						))}
				</Stack>
			</Collapse>
...
const MobileMenu = ({ menu = null, showSocialLinks = false, socialLinks = null }) => {
	return (
		<Stack bg={useColorModeValue('whiteAlpha.600', 'blackAlpha.600')} p={4} display={{ md: 'none' }}>
			{menu &&
