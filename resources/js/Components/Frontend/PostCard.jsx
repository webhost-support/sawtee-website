...
import { LinkBox, Box, Flex, Heading, Text, Image, HStack, useColorModeValue } from '@chakra-ui/react';
...
function PostCard({ post, ...rest }) {
	const { headingSize, showDate, showDescription, showImage = true, showTags, imageHeadgingGap, aspect } = { ...rest };
	const color = useColorModeValue('var(--color-dark', 'var(--color-light)');
	const featured_image =
		post.media.length > 0 ? post.media.filter(item => item.collection_name === 'post-featured-image')[0] : null;
	const srcSet = featured_image ? featured_image.responsive_images.responsive.urls : null;
	return (
		<Box>
				...
				{showImage && (
					...
					>
						<Image
							...
							src={featured_image ? featured_image.original_url : '/assets/SM-placeholder-150x150.png'}
