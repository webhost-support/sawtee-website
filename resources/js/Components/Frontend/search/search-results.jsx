...
const SearchHeader = ({ label, title, ...props }) => (
	<Box {...props}>
		<Heading size="md" as="h6" fontWeight="medium" textTransform="uppercase" color="accent.400">
			{label}
		</Heading>

		<Heading mt={4} fontWeight="medium" as="h1" textTransform="uppercase" fontSize="3.5rem" color="white">
			{title}
		</Heading>
	</Box>
);
...
const SearchResultTitle = ({ resultCount, query }) => (
	<SearchHeader title={`‘${query}’`} label={`${resultCount} ${resultCount > 1 ? 'results' : 'result'} for`} />
);

const NoResultTitle = ({ query }) => <SearchHeader label={`0 result for`} title={`‘${query}’`} />;
			...
			<PatternBox showPattern={false} pb="60px" border={'none'}>
				<PatternBoxInner>
					{isEmpty ? (
						...
						<SearchResultTitle query={query.replace(/\+/g, ' ')} resultCount={data.length} />
					)}
				</PatternBoxInner>
				<SimpleGrid columns={{ base: 1, md: 2 }} maxW={'7xl'} spacing={10} mx="auto" px={{ base: '24px', md: '80px' }}>
					{data &&
						data.map(post => {
							console.log(post);
							return (
