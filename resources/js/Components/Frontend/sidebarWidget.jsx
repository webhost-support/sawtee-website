import { Box, Skeleton, ListItem, useColorModeValue, Text, Flex } from '@chakra-ui/react';
	...
	return (
		...
		>
			<SimpleList heading={title} px="8" py={4} spacing={4}>
					...
					array.map((post, index) => {
						return (
							<ListItem key={post.id} mb="1rem">
								<Box>
									<ChakraLink as={Link} textDecor="underline" textUnderlineOffset="3px" href={`${link}/${post.slug}`}>
				...
				<ExploreButton size={['xs', 'sm']} text={`More ${title}`} link={link} p={0} />
			</SimpleList>
		</GlassBox>
	);
};
