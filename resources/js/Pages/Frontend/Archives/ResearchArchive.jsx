...
const ResearchArchive = ({ posts }) => {
	...
	if (!posts || posts.length <= 0) return <Text fontSize={'2xl'}>"No posts found"</Text>;
								...
								{tagitem[1].map((researchItem, idx) => (
									...
									>
										<Text fontSize="sm">
											<Link
												target="_blank"
												href={researchItem.file ? `/Research_Reports/${researchItem.file.name}` : researchItem.link}
											>
												{researchItem.title}
											</Link>
										</Text>
	...
	return (
		<Flex {...props} align={'center'}>
			<Flex flexDir="column" alignItems="center" mr={4} pos="relative">
				...
				<Box as={icon} size="1.25em" pos="absolute" left="0.85em" top="0.85em" />
				{skipTrail ? (
					<Box
						w="1px"
						flex={1}
