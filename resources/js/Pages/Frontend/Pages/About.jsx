...
export const PageSection = ({ section, sections }) => {
	...
	return (
		<Box id={sectionID}>
			<Heading as="h2" fontSize={['lg', 'xl', '2xl']} py={'4'} mb="4" fontFamily="heading">
				{title}
			</Heading>
			...
			{isTabs && childSections.length > 0 && (
				<Box px="6" py="4">
					<Tabs variant="enclosed" isFitted>
						...
						<TabPanels border="1px solid" borderColor={useColorModeValue('gray.700', 'gray.300')}>
							{childSections.map(({ description, title }) => (
								<TabPanel key={title} bg={tabColor} p={6} transition="all 0.4s ease-in">
									<Box
										display="flex"
