...
export default function Sidebar({ isCollapsed, menu }) {
  const { url } = usePage();
  return (
    <aside className="ease-nav-brand my-4 block w-full max-h-screen flex-wrap items-center justify-between rounded-2xl bg-white p-0 antialiased shadow-none border-r-slate-200 xl:left-0 xl:translate-x-0 xl:bg-transparent text-slate-500 transition-all duration-300 ">
      ...
      <ScrollArea className="items-center block w-auto h-4/5 grow basis-full">
            ...
            return isCollapsed ? (
              ...
              >
                <Link
                  href={route(menuItem.route)}
                  className={
                    'flex justify-start items-center px-2 gap-2 text-muted-foreground hover:text-foreground dark:bg-muted dark:text-muted-foreground dark:hover:bg-muted dark:hover:text-white'
                  }
                >
